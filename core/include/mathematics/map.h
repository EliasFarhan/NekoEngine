...
public:
    ...
    Map(Allocator* allocator, const size_t sizeInBytes)     //Todo Replace by capacity && allocator reference not pointer
    {
        (void)allocator; // TODO: Pass this allocator to neko's container.
    ...
    bool Add(const Key key, const Value value)
    {
        ...
        for (size_t i = 0; i < len; i++)        //todo std::find_if
        {
            if (pairs_[i].first == 0)
            {
                pairs_[i].first = hash;
    ...
    bool Remove(const Key key)
    {
        const xxh::hash_t<64> hash = xxh::xxhash<64>(&key, sizeof(Key));
        const size_t len = pairs_.capacity();
        for (int i = 0; i < len; i++)           //todo find_if
        {
    ...
    void Clear()
    {
        for (auto& pair : pairs_)           //todo std::for_each
        {
            pair = std::pair<xxh::hash_t<64>, Value>(0, Value());
        }
        count_ = 0;
    ...
    bool Swap(const Key a, const Key b)         //todo remove
    {
        ...
        do
        {
            ...
            if (hash == hash0 || hash == hash1)
            {
                ...
                {
                    index1 = i;
                    goto RETURN;            //Todo: remove goto
                }
            }
        } while (i++ < len);        //todo:std::find_if
